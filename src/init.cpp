...
namespace startup
{
	namespace 
	{
		#define in_the_forest std::vector<const wstring>{ L"in het bos", L"in the forest" }
		#define in_the_swamp std::vector<const wstring>{ L"in het moeras", L"in the swamp" }
		#define at_an_open_space std::vector<const wstring>{ L"op een open plek", L"at an open space" }
		#define	in_an_empty_cave std::vector<const wstring>{ L"in een lege grot", L"in an empty cave" }
		#define	in_a_stairwell_cave std::vector<const wstring>{ L"in een wenteltrapgrot", L"in a stairwell cave" }
		#define smelly_surface std::vector<const wstring>{ L"Er hangt een griezelige nevel over het stinkende oppervlak.", L"A creepy mist hangs over the smelly surface." }
		#define	other_levels std::vector<const wstring>{ L"In deze grot bevindt zich een grote, stenen wenteltrap die waarschijnlijk naar andere niveaus leidt.", L"There is a large, circular stairwell that is made out of stone. It looks like it leads to other levels." }

		std::unique_ptr<Room> rooms[] { // { names, descriptions }
			std::make_unique<Forest>(
				in_the_forest,
				std::vector<const wstring>{
			...
			std::make_unique<Outdoor>(
				std::vector <const wstring>{ L"op de begraafplaats", L"at the cemetery" },
				std::vector <const wstring>{
			...
			std::make_unique<Outdoor>(
				std::vector<const wstring>{ L"op het moeraspad", L"on the swamp path" },
				std::vector<const wstring>{
			...
			std::make_unique<Indoor>(
				std::vector<const wstring>{ L"in de ruïne", L"in the ruins" },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de slijmgrot", L"in the slime cave" },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de zwarte grot", L"in the black cave" },
				std::vector<const wstring>{
					L"Je ziet geen hand voor ogen.",
					L"You can't see your hand in front of your face."
				}
			),
			std::make_unique<Cave>(std::vector <const wstring>{ L"in de drugsgrot", L"in the drugs cave" }),
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de geile grot", L"in the horny cave", },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de dwangbuisgrot", L"in the straitjacket cave" },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(std::vector<const wstring>{ L"in een verwaarloosde grot", L"in a neglected cave" }),
			std::make_unique<Cave>(in_an_empty_cave),
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de hoofdgrot", L"in the main cave" },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de hiërogliefengrot", L"in the hieroglyphs cave" },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de stankgrot", L"in the stench cave" },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(std::vector<const wstring>{ L"in een troosteloze grot", L"in a disconsolate cave" }),
			std::make_unique<Indoor>(
				std::vector<const wstring>{ L"in de TL-grot", L"in the fluorescent cave" },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(std::vector<const wstring>{ L"in een kleine grot", L"in a small cave" }),
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de ijsgrot", L"in the ice cave" },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de kaktusgrot", L"in the cactus cave", },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de stalagmietengrot", L"in the stalagmite cave" },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de stormgrot", L"in the storm cave" },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(std::vector<const wstring>{ L"in de mistgrot", L"in the mist cave" }),
			std::make_unique<Cave>(in_a_stairwell_cave, other_levels),
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de tentakelgrot", L"in the tentacle cave" },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de vuilnisgrot", L"in the garbage cave" },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de echogrot", L"in the echo cave" },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de geheime grot", L"in the secret cave" },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de voedselgrot", L"in the food cave" },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de grot van de gnoe", L"in the cave of the gnu" },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(in_an_empty_cave),
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de ogengrot", L"in the eye cave" },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de rotsgrot", L"in the rock cave" },
				std::vector<const wstring>{
			...
			std::make_unique<Outdoor>(std::vector<const wstring>{ L"in de leegte", L"in the emptiness" }),
			std::make_unique<Outdoor>(
				std::vector<const wstring>{ L"op de zandbank", L"at the sandbank" },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de martelgrot", L"in the torture cave" },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(in_an_empty_cave),
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de veilige grot", L"in the safe cave" },
				std::vector<const wstring>{
			...
			std::make_unique<Outdoor>(
				std::vector<const wstring>{ L"in de nauwe rotsspleet", L"in the narrow cleft" },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de oliegrot", L"in the oil cave" },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(in_a_stairwell_cave, other_levels),
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de spinnengrot", L"in the spider cave" },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(std::vector<const wstring>{ L"in de pratende grot", L"in the talking cave" }),
			std::make_unique<Cave>(std::vector<const wstring>{ L"op de lavaput", L"at the lava pit" }),

			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de skoebiegrot", L"in the scooby cave" },
				std::vector<const wstring>{
			...
			std::make_unique<Indoor>(
				std::vector<const wstring>{ L"in de radioactieve grot", L"in the radioactive cave" },
				std::vector<const wstring>{
					L"Door de hoge radioactiviteit is deze grot zwak verlicht.",
					L"Due to the high level of radioactivity, this cave is dimly lit."
				}
			),
			std::make_unique<Cave>(std::vector<const wstring>{ L"in de i-grot", L"in the second t cave" }),
			std::make_unique<Cave>(std::vector<const wstring>{ L"in de p-grot", L"in the first t cave" }),
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de a-grot", L"in the first e cave" },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de dodengrot", L"in the cave of death" },
				std::vector<const wstring>{
					L"De naam van deze grot spreekt voor zich...",
					L"The name of this cave speaks for itself..."
				}
			),
			std::make_unique<Cave>(std::vector<const wstring>{ L"in de r-grot", L"in the r cave" }),
			std::make_unique<Cave>(std::vector<const wstring>{ L"in de e-grot", L"in the second e cave" }),
			std::make_unique<Cave>(in_a_stairwell_cave, other_levels),
			std::make_unique<Cave>(std::vector<const wstring>{ L"in de P-grot", L"in the L cave" }),
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de grot der verdoemenis", L"in the damnation cave" },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(std::vector<const wstring>{ L"in de vacuumgrot", L"in the vacuum cave" }),
			std::make_unique<Cave>(std::vector<const wstring>{ L"in de rode grot", L"in the red cave" }),
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de neongrot", L"in the neon cave" },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de bloedgrot", L"in the blood cave" },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de vleermuisgrot", L"in the bat cave" },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de slangengrot", L"in the snake cave" },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(std::vector<const wstring>{ L"in de kwabbengrot", L"in the lobe cave" }),
			std::make_unique<Cave>(
				std::vector<const wstring>{ L"in de glibbergrot", L"in the slippery cave" },
				std::vector<const wstring>{
			...
			std::make_unique<Cave>(std::vector<const wstring>{ L"in de teleportgrot", L"in the teleport cave" })
		};
		...
		std::unique_ptr<Item> items[] { // { name, description, room, useable_on }
			std::make_unique<HoundMeat>(
				std::vector<const string>{ "het hellehondvlees", "the hell hound meat" },
				std::vector<const wstring>{
			...
			std::make_unique<ThermalSuit>(
				std::vector<const string>{ "het hittepak", "the thermal suit" },
				std::vector<const wstring>{
			...
			std::make_unique<Crystal>(
				std::vector<const string>{ "het groen kristal", "the green crystal" },
				std::vector<const wstring>{
			...
			std::make_unique<Sword>(
				std::vector<const string>{ "het zwaard", "the sword" },
				std::vector<const wstring>{
			...
			std::make_unique<Bone>(
				std::vector<const string>{ "het bot", "the bone" },
				std::vector<const wstring>{
			...
			std::make_unique<Diskette>(
				std::vector<const string>{ "de diskette", "the diskette" },
				std::vector<const wstring>{
			...
			std::make_unique<Hashis>(
				std::vector<const string>{ "de hasj", "the hashis" },
				std::vector<const wstring>{
			...
			std::make_unique<Crystal>(
				std::vector<const string>{ "het rood kristal", "the red crystal" },
				std::vector<const wstring>{
			...
			std::make_unique<Nightcap>(
				std::vector<const string>{ "de slaapmuts", "the nightcap" },
				std::vector<const wstring>{
			...
			std::make_unique<Bomb>(
				std::vector<const string>{ "de noekietronenbom", "the nookytron bomb" },
				std::vector<const wstring>{
			...
			std::make_unique<Flashlight>(
				std::vector<const string>{ "de zaklamp", "the flashlight" },
				std::vector<const wstring>{
			...
			std::make_unique<Bandage>(
				std::vector<const string>{ "het verband", "the bandage kit" },
				std::vector<const wstring>{
			...
			std::make_unique<Flamethrower>(
				std::vector<const string>{ "de vlammenwerper", "the flamethrower" },
				std::vector<const wstring>{
			...
			std::make_unique<Item>(
				std::vector<const string>{ "het kookboek", "the cookbook" },
				std::vector<const wstring>{
			...
			std::make_unique<TNT>(
				std::vector<const string>{ "de tnt", "the tnt" },
				std::vector<const wstring>{
			...
			std::make_unique<CombinableItem>(
				std::vector<const string>{ "het mosterdgaspatroon", "the mustard gas cartrtridge" },
				std::vector<const wstring>{
			...
			std::make_unique<PoisonedMeat>(
				std::vector<const string>{ "het giftige vlees", "the poisonous meat" },
				std::vector<const wstring>{
			...
			std::make_unique<CombinableItem>(
				std::vector<const string>{ "de ontsteking", "the ignition" },
				std::vector<const wstring>{
			...
			std::make_unique<CombinableItem>(
				std::vector<const string>{ "het pakje batterijen", "the pack of batteries" },
				std::vector<const wstring>{
			...
			std::make_unique<Gasmask>(
				std::vector<const string>{ "het gasmasker", "the gasmask" },
				std::vector<const wstring>{
			...
			std::make_unique<Item>(
				std::vector<const string>{ "het papier", "the letter" },
				std::vector<const wstring>{
			...
			std::make_unique<Booklet>(
				std::vector<const string>{ "het boekje", "the booklet" },
				std::vector<const wstring>{
			...
			std::make_unique<Crystal>(
				std::vector<const string>{ "het blauw kristal", "the blue crystal" },
				std::vector<const wstring>{
			...
			std::make_unique<Cookie>(
				std::vector<const string>{ "het koekje", "the cookie" },
				std::vector<const wstring>{
			...
			std::make_unique<GasGrenade>(
				std::vector<const string>{ "de mosterdgasgranaat", "the mustard gas grenade" },
				std::vector<const wstring>{
